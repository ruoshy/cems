        ...
        render(row, index) {
          return h(NSpace, {
            ...
            h(NSwitch, {
              ...
              value: row.lock,
              onClick: () => {
                let item = pag.invigilate.data[index]
                console.log(!item.lock)
                pag.invigilate.data[index].lock = !item.lock
                manage.updateInvigilate({
                  account: {
                    id: item.account_id,
                    lock: pag.invigilate.data[index].lock
                  }
                }).then(res => {
                  let json = res.data
                  if (json.state === 200) {
...
const modal = reactive({
  add: {
    ...
    confirm: () => {
      ...
      manage.addInvigilate(modal.data).then(res => {
        let json = res.data
        if (json.state === 200) {
  ...
  update: {
    ...
    confirm: () => {
      ...
      manage.updateInvigilate(data).then(res => {
        let json = res.data
        if (json.state === 200) {
...
function updateInvigilate(page) {
  if (page)
    pag.invigilate.current = page
  manage.getInvigilateList({page: pag.invigilate.current, size: pag.invigilate.size}).then(res => {
    if (res.data.state === 200) {
      ...
      res.data.data.list.forEach(item => {
        data.push({
          ...
          key: (pag.invigilate.current - 1) * pag.invigilate.size + index++,
          name: item.name,
      ...
      pag.invigilate.data = data
      pag.invigilate.count = res.data.data.pages === 0 ? 1 : res.data.data.pages
    }
  })
}
...
<template>
  <n-card>
    <n-space vertical :size="[0,20]">
      ...
      <n-data-table :columns="pag.invigilate.columns" :data="pag.invigilate.data"/>
      <n-pagination v-model:page="pag.invigilate.current" :page-count="pag.invigilate.count"
                    :on-update:page="updateInvigilate"/>
    </n-space>
  </n-card>
